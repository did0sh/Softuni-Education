...
public class BoatSimulatorControllerImpl implements BoatSimulatorController {
    ...
    public static final String YACHT_WITH_MODEL_S_REGISTERED_SUCCESSFULLY = "Yacht with model %s registered successfully.";
    ...
    public String create(Boat boat) throws DuplicateModelException {
        ...
        int otherIndex = className.indexOf("Yacht");
        if (index != -1) {

            return String.format(BOAT_WITH_MODEL_S_REGISTERED_SUCCESSFULLY,
                    className.substring(0, index), boat.getModelName());
        } else if (otherIndex != -1){
            return String.format(YACHT_WITH_MODEL_S_REGISTERED_SUCCESSFULLY, boat.getModelName());
        }
        return null;
    }
